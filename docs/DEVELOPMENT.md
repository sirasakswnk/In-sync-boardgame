...
## แก้คำถามและรูปบนการ์ด

คำถามทั้งหมดอยู่ใน [src/data/questions.th.json](../src/data/questions.th.json) แก้เสร็จแล้วรัน `npm run check:questions` และ `npm test`

**รูปบนการ์ดตัวเลือก** ใช้ได้ทั้งอีโมจิและรูปวาดเอง ปนกันในคำถามเดียวกันได้

1. วางไฟล์ไว้ที่ `public/icons/` ตั้งชื่อด้วยตัวอังกฤษ ตัวเลข `-` หรือ `_` เช่น `q31-o1.svg`
2. ใส่ path ในช่อง `icon` ของตัวเลือก เช่น `"icon": "/icons/q31-o1.svg"` (ขึ้นต้นด้วย `/icons/` ไม่ต้องมี `public`)

| ไฟล์ | ใช้เมื่อ | ขนาดที่ควร export |
| --- | --- | --- |
| **SVG** (แนะนำ) | ภาพแบนสีเรียบ เส้นชัด | canvas จัตุรัส เช่น 64×64 |
| **WebP** | ภาพวาดมีพื้นผิว ไล่สี | 256×256 px พื้นหลังโปร่งใส ไม่เกินราว 20 KB |
| **PNG** | export WebP ไม่ได้ | 256×256 px พื้นหลังโปร่งใส |

- รูปแสดงขนาดเท่าอีโมจิในทุกหน้า (ไพ่ในมือ ไพ่บนแท่น หน้ารอ หน้าเฉลย ตารางย้อนดู) จึงไม่ต้องทำหลายขนาด
- ควรเป็นจัตุรัสและเว้นขอบใสเท่ากันทุกรูป ไพ่จะดูสมดุล
- `check:questions` และเทสต์จะล้มถ้า path ผิดรูปแบบหรือไม่มีไฟล์อยู่จริง จึงไม่มีทางหลุดไปเป็นรูปเสียบนเว็บ
...
public/icons/               รูปวาดเองบนการ์ดตัวเลือก
